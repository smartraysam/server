...
static gxObject *ALL_OBJECTS[] = {BASE(associationNone), BASE(associationLow), BASE(associationHigh), BASE(associationHighGMac), BASE(securitySetupLow), BASE(securitySetupHigh),
                                  BASE(ldn), BASE(sapAssignment), BASE(eventCode), BASE(meterData.clock1), BASE(autoConnect), BASE(modemConfiguration), BASE(gprsSetup), 
                                  BASE(totalreactiveEnergy), BASE(totalapparentEnergy), BASE(totalactiveEnergy), BASE(activeEnergyL1), BASE(activeEnergyL2), BASE(activeEnergyL3),
                                  BASE(totalactivePower), BASE(activePowerL1), BASE(activePowerL2), BASE(activePowerL3),
                                  BASE(instVoltage), BASE(instVoltageL1), BASE(instVoltageL2), BASE(instVoltageL3), BASE(instCurrent), BASE(instCurrentL1), BASE(instCurrentL2), BASE(instCurrentL3),
                                  BASE(powerFactor), BASE(powerFactorL1), BASE(powerFactorL2), BASE(powerFactorL3), BASE(frequency), BASE(energyConstant), BASE(firmware),
                                  BASE(pushSetup), BASE(scriptTableGlobalMeterReset), BASE(scriptTableDisconnectControl),
...
int addAssociationLow(uint16_t serializationVersion)
{
  ...
  if ((ret = INIT_OBJECT(associationLow, DLMS_OBJECT_TYPE_ASSOCIATION_LOGICAL_NAME, ln)) == 0)
  {
    ...
    {
      memcpy(meterData.association.llsPassword, "@smartSusej", 11);
      meterData.association.llsPasswordSize = 11;
    }
...
int addAssociationHigh(uint16_t serializationVersion)
{
  ...
  if ((ret = INIT_OBJECT(associationHigh, DLMS_OBJECT_TYPE_ASSOCIATION_LOGICAL_NAME, ln)) == 0)
  {
    ...
    if (serializationVersion == 0)
    {
      memcpy(meterData.association.hlsPassword, "@smartSusej", 11);
      meterData.association.hlsPasswordSize = 11;
    }
...
int addModemConfiguration()
{
  ...
  {
    modemConfiguration.communicationSpeed = DLMS_BAUD_RATE_9600;
...
int addLoadProfileProfileGeneric()
{
  int ret;
  //Two capture objects.
  static gxTarget CAPTURE_OBJECT[26] = {0};
...
void createObjects()
{
  ...
  if ((ret = addLogicalDeviceName()) != 0 ||
      ...
      (ret = addTReactiveEnergyRegisterObject()) != 0 ||
      (ret = addTApparentEnergyRegisterObject()) != 0 ||
      (ret = addTActiveEnergyRegisterObject()) != 0 ||
      (ret = addActiveEnergyL1RegisterObject()) != 0 ||
      (ret = addActiveEnergyL2RegisterObject()) != 0 ||
      (ret = addActiveEnergyL3RegisterObject()) != 0 ||
      (ret = addActiveEnergyL1RegisterObject()) != 0 ||
      (ret = addTActivePowerRegisterObject()) != 0 ||
      (ret = addActivePowerL1RegisterObject()) != 0 ||
      (ret = addActivePowerL2RegisterObject()) != 0 ||
      (ret = addActivePowerL3RegisterObject()) != 0 ||
      (ret = addVoltageRegisterObject()) != 0 ||
      (ret = addVoltageL1RegisterObject()) != 0 ||
      (ret = addVoltageL2RegisterObject()) != 0 ||
      (ret = addVoltageL3RegisterObject()) != 0 ||
      (ret = addCurrentRegisterObject()) != 0 ||
      (ret = addCurrentL1RegisterObject()) != 0 ||
      (ret = addCurrentL2RegisterObject()) != 0 ||
      (ret = addCurrentL3RegisterObject()) != 0 ||
      (ret = addPFRegisterObject()) != 0 ||
      (ret = addPFL1RegisterObject()) != 0 ||
      (ret = addPFL2RegisterObject()) != 0 ||
      (ret = addPFL3RegisterObject()) != 0 ||
      (ret = addFregRegisterObject()) != 0 ||
      (ret = addFirmVerRegisterObject()) != 0 ||
      (ret = addECRegisterObject()) != 0 ||
...
void setup()
{
  ...
  DLMS_COM.begin(getCommunicationSpeed());
  while (!DLMS_COM)
  {
...
void loop()
{
  ...
  if (available > 0)
  {
    ...
    available = DLMS_COM.readBytes(tmp, available);
    ...
    if (reply.size != 0)
    {
      //Send reply. via TCP(GSM module), or Serial
      DLMS_COM.write(reply.data, reply.size);
      DLMS_COM.flush();
      bb_clear(&reply);
    }
  }
    ...
    gxValueEventCollection *args)
{
  ...
  for (pos = 0; pos != args->size; ++pos)
  {
    ...
    if (e->target == &totalreactiveEnergy.base && e->index == 2)
    {
      readReactiveEnergyValue();
    }
    if (e->target == &totalapparentEnergy.base && e->index == 2)
    {
      readApparentEnergyValue();
    }
    if (e->target == &totalactivePower.base && e->index == 2)
    {
      readTotalActivePowerValue();
    }
    if (e->target == &activePowerL1.base && e->index == 2)
    {
      readActivePowerL1Value();
    }
    if (e->target == &activePowerL2.base && e->index == 2)
    {
      readActivePowerL2Value();
    }
    if (e->target == &activePowerL3.base && e->index == 2)
    {
      readActivePowerL3Value();
    }
    if (e->target == &instVoltage.base && e->index == 2)
    {
      readVoltageValue();
    }
    if (e->target == &instVoltageL1.base && e->index == 2)
    {
      readVoltageL1Value();
    }
    if (e->target == &instVoltageL2.base && e->index == 2)
    {
      readVoltageL2Value();
    }
    if (e->target == &instVoltageL3.base && e->index == 2)
    {
      readVoltageL3Value();
    }
    if (e->target == &instCurrent.base && e->index == 2)
    {
      readCurrentValue();
    }
    if (e->target == &instCurrentL1.base && e->index == 2)
    {
      readCurrentL1Value();
    }
    if (e->target == &instCurrentL2.base && e->index == 2)
    {
      readCurrentL2Value();
    }
    if (e->target == &instCurrentL3.base && e->index == 2)
    {
      readCurrentL3Value();
    }
    if (e->target == &powerFactor.base && e->index == 2)
    {
      readpowerFactorValue();
    }
    if (e->target == &powerFactorL1.base && e->index == 2)
    {
      readpowerFactorL1Value();
    }
    if (e->target == &powerFactorL2.base && e->index == 2)
    {
      readpowerFactorL2Value();
    }
    if (e->target == &powerFactorL3.base && e->index == 2)
    {
      readpowerFactorL3Value();
    }
    if (e->target == &totalactiveEnergy.base && e->index == 2)
    {
      readTotalActiveEnergyValue();
    }
    if (e->target == &activeEnergyL1.base && e->index == 2)
    {
      readActiveEnergyL1Value();
    }
    if (e->target == &activeEnergyL2.base && e->index == 2)
    {
      readActiveEnergyL2Value();
    }
    if (e->target == &activeEnergyL3.base && e->index == 2)
    {
      readActiveEnergyL3Value();
    }
    if (e->target == &frequency.base && e->index == 2)
    {
      readFreqValue();
    }

    if (e->target == &energyConstant.base && e->index == 2)
    {
      readECValue();
    }

    if (e->target == &firmware.base && e->index == 2)
    {
      readFirmwareValue();
    }
    ...
    gxPushSetup *push)
{
  ...
  if ((ret = Server.generatePushSetupMessages(push, &messages)) == 0)
  {
    ...
    {
      DLMS_COM.write(bb.data, bb.size);
      DLMS_COM.flush();
    }
  }
  if (ret != 0)
    ...
    gxValueEventArg *it)
{
  ...
  else if (it->index == 2)
  {
    ...
    row.time = time_current();
    row.totalreactiveEnergy = readReactiveEnergyValue();
    row.totalapparentEnergy = readApparentEnergyValue();
    row.totalactivePower = readTotalActivePowerValue();
    row.activePowerL1 = readActivePowerL1Value();
    row.activePowerL2 = readActivePowerL2Value();
    row.activePowerL3 = readActivePowerL3Value();
    row.totalactiveEnergy = readTotalActivePowerValue();
    row.activeEnergyL1 = readActiveEnergyL1Value();
    row.activeEnergyL2 = readActiveEnergyL2Value();
    row.activeEnergyL3 = readActiveEnergyL3Value();
    row.instVoltage = readVoltageValue();
    row.instVoltageL1 = readVoltageL1Value();
    row.instVoltageL2 = readVoltageL2Value();
    row.instVoltageL3 = readVoltageL3Value();
    row.instCurrent = readCurrentValue();
    row.instCurrentL1 = readCurrentL1Value();
    row.instCurrentL2 = readCurrentL2Value();
    row.instCurrentL3 = readCurrentL3Value();
    row.powerFactor = readpowerFactorValue();
    row.powerFactorL1 = readpowerFactorL1Value();
    row.powerFactorL2 = readpowerFactorL2Value();
    row.powerFactorL3 = readpowerFactorL3Value();
    row.frequency = readFreqValue();
    row.energyConstant = readECValue();
    row.firmware = readFirmwareValue();
    ...
    gxValueEventCollection *args)
{
  ...
  for (pos = 0; pos != args->size; ++pos)
  {
    ...
    else if (e->target == BASE(totalreactiveEnergy))
    {
      //Set default value for active power.
      totalreactiveEnergyVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(totalapparentEnergy))
    {
      //Set default value for active power.
      totalapparentEnergyVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(totalactiveEnergy))
    {
      //Set default value for active power.
      totalactiveEnergyVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(activeEnergyL1))
    {
      //Set default value for active power.
      activeEnergyL1Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(activeEnergyL2))
    {
      //Set default value for active power.
      activeEnergyL2Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(activeEnergyL3))
    {
      //Set default value for active power.
      activeEnergyL3Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(totalactivePower))
    {
      //Set default value for active power.
      t_activePowerVal = 0;
      e->handled = 1;
    ...
    else if (e->target == BASE(activePowerL2))
    {
      //Set default value for active power.
      activePowerL2Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(activePowerL3))
    {
      //Set default value for active power.
      activePowerL3Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instVoltage))
    {
      //Set default value for active power.
      instVoltageVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instVoltageL1))
    {
      //Set default value for active power.
      instVoltageL1Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instVoltageL2))
    {
      //Set default value for active power.
      instVoltageL2Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instVoltageL3))
    {
      //Set default value for active power.
      instVoltageL3Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instCurrent))
    {
      //Set default value for active power.
      instCurrentVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instCurrentL1))
    {
      //Set default value for active power.
      instCurrentL1Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instCurrentL2))
    {
      //Set default value for active power.
      instCurrentL2Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(instCurrentL3))
    {
      //Set default value for active power.
      instCurrentL3Val = 0;
      e->handled = 1;
    ...
    //
    else if (e->target == BASE(powerFactor))
    {
      //Set default value for active power.
      powerFactorVal = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(powerFactorL1))
    {
      //Set default value for active power.
      powerFactorL1Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(powerFactorL2))
    {
      //Set default value for active power.
      powerFactorL2Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(powerFactorL3))
    {
      //Set default value for active power.
      powerFactorL3Val = 0;
      e->handled = 1;
    }
    else if (e->target == BASE(frequency))
    {
      //Set default value for active power.
      frequencyVal = 0;
      e->handled = 1;
    ...
    else if (e->target == BASE(energyConstant))
    {
      //Set default value for active power.
      energyConstantVal = 0;
      e->handled = 1;
    ...
    else if (e->target == BASE(firmware))
    {
      //Set default value for active power.
      firmwareVal = 0;
      e->handled = 1;
    ...
    else if (e->target == BASE(autoConnect))
    {
      //Set default value for active power.
    ...
    else if (e->target == BASE(modemConfiguration))
    {
      //Set default value for active power.
    ...
    else if (e->target == BASE(gprsSetup))
    {
      //Set default value for active power.
